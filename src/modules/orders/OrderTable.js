            ...
            <div className="relative overflow-x-auto">
                <table className="text-left md:w-full">
                    <thead className="bg-surface border-b border-outline-variant text-[12px] font-bold text-secondary uppercase tracking-wider">
                        <tr>
                            <th className="px-5 py-4"><p className='w-max'>{sTrans("Mã đơn hàng")}</p></th>
                            ...
                            orders.map((order) => (
                                <tr className="transition-colors hover:bg-surface-container-low group" key={order?._id}>
                                    <td className="px-5 py-4">
                                        <p className={`w-[250px] line-clamp-1 break-all overflow-hidden ${order?.paid ? 'text-green-700' : 'text-red-500'}`} title={order?._id || sTrans("ID trống")}>
                                            {order?._id || <span className='italic text-secondary'>{sTrans("ID trống")}</span>}
                                        </p>
                                    </td>
                                    <td className="px-5 py-4 text-on-surface ">
                                        <div className='flex flex-col gap-2'>
                                            <p className="w-[150px] line-clamp-1 break-all overflow-hidden" title={order?.userName || sTrans("Không tên")}>
                                                {order?.userName || <span className='italic text-secondary'>{sTrans("Không tên")}</span>}
                                            </p>
                                    ...
                                    <td className="px-5 py-4">
                                        <p className='w-[100px]'>
                                            {order?.deliveryInfo?.mode === "delivery" ? sTrans("Giao hàng") : sTrans("Mua mang về")}
                                        </p>
                                    </td>
                                    <td className="sticky right-0 z-10 px-6 py-4 text-right bg-white ">
                                        <div className="flex justify-end gap-3">
                                            <Link href={`${ORDERS_ROUTE}/${order._id}?from=orders`} className={`transition-colors text-secondary hover:text-primary ${loadingForm ? "opacity-50 cursor-not-allowed" : ""}`} ><HiDotsHorizontal className="w-5 h-5" title={sTrans("Chi tiết")} /></Link>
                                        </div>
                                    </td>
                                </tr>
                            )) : (
