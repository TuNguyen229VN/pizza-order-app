                        ...
                        <tr className="transition-colors hover:bg-surface-container-low group" key={user?._id}>
                            ...
                            <td className="sticky right-0 z-10 px-6 py-4 text-right bg-white ">
                                <div className="flex justify-end gap-3">
                                    <Link href={`${USERS_ROUTE}/${user?._id}`} className={`transition-colors text-secondary hover:text-primary ${loadingForm ? "opacity-50 cursor-not-allowed" : ""}`}><MdOutlineModeEdit className="w-5 h-5" /></Link>
                                    {!user?.admin && <ConfirmPopup disabled={loadingForm} label={`${user?.status === "on" ? "Bỏ chặn" : "Chặn"} ${user?.name || "người dùng"}`} labelConfirm={user?.status === "on" ? "Bỏ chặn" : "Chặn"} onDelete={() => { handleUserBlock(user) }}>
