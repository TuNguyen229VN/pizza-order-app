        ...
        return createPortal(
            <div className='fixed inset-0 z-50 flex items-center justify-center h-full bg-black/80'>
                <div className="flex flex-col items-center justify-center p-6 text-base bg-white shadow-lg rounded-2xl max-w-[960px] w-[345px]">
                    <p className='font-semibold text-center'>{sTrans.has(label) ? sTrans(label) : label}</p>
                    <div className='w-1/3 h-1 mt-2 mb-4 rounded-lg bg-primary'></div>
                    <div className="text-sm font-normal text-center">{(label.includes("Thoát")||label.includes("Exit")) ? (sTrans.has(labelDesc) ? sTrans(labelDesc) : labelDesc) : sTrans("Bạn có muốn")}{(!label.includes("Thoát")&&!label.includes("Exit")) && <span className='lowercase'>{labelDesc ? (sTrans.has(labelDesc) ? sTrans(labelDesc) : labelDesc) : (sTrans.has(label) ? sTrans(label) : label)}</span>} {sTrans("này không")}?</div>
                    <div className='flex gap-4 mt-4'>
                        ...
                        <button className='px-6 py-3 text-white rounded-lg bg-primary w-[125px] hover:opacity-80 hover:scale-[1.02] duration-500' onClick={() => {
                            ...
                            onDelete();
                        }}>{sTrans.has(labelConfirm) ? sTrans(labelConfirm) : labelConfirm}</button>
                    </div>
                </div>
            </div>
            , document.body)
